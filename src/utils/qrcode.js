...
// webpack.config.mjs sets a static (non-"auto") `output.publicPath` to avoid a
// runtime helper that AMO's linter flags ("The Function constructor is eval").
// A static build-time value can't know the extension's root URL though (this
// module gets bundled into pages nested under pages/ as well as the top-level
// background script), so point webpack's chunk loader at the actual extension
// root before the `import()`/`importScripts()` below ever runs. This must
// execute before `initDecoder()` is called, which it does: ES module bodies
// run top-to-bottom on first import, before any importer's own top-level code.
// eslint-disable-next-line no-undef
__webpack_public_path__ = apiNs.runtime.getURL("/");
