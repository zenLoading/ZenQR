...
function generateConfig(browser, debug) {
  ...
  return {
    ...
    output: {
      ...
      clean: true,
      // Default publicPath ("auto") makes webpack inject a runtime helper that
      // inspects `document.currentScript`/`self.location`, which falls back to
      // `new Function("return this")()` to locate the global object — AMO's
      // linter flags that as eval. A static empty publicPath (resolve chunk
      // URLs relative to the loading script, which is how extension pages and
      // the background script already work) removes the need for that helper.
      publicPath: "",
    },
    plugins: [
      new CopyPlugin({
      ...
      new webpack.DefinePlugin({
        ...
        // can't otherwise find the global object — AMO's linter flags that as
        // eval. Substituting `globalThis` (always present in the Firefox/Chrome
        // versions this extension targets) at compile time removes the need for
        // that shim entirely.
        global: "globalThis",
      }),
